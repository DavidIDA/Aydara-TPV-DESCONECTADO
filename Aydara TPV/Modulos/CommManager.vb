...
#Region "Manager Variables"
    ...
    Public _displayValue As String
...
#Region "Manager Constructors"
    ...
    Public Sub New(ByVal baud As String, ByVal par As String, ByVal sBits As String, ByVal dBits As String, ByVal name As String, ByVal value As String)
        _baudRate = baud
        _parity = par
        _stopBits = sBits
        _dataBits = dBits
...
#Region "DisplayData"
    ...
    Private Sub DisplayData(ByVal type As MessageType, ByVal msg As String)
        '_displayValue.Invoke(New EventHandler(AddressOf DoDisplay))
        DoDisplay(Nothing, Nothing)
    End Sub
#End Region
...
#Region "OpenPort"
    Public Function OpenPort() As Boolean
        Try
            ...
            If _baudRate <> String.Empty Then comPort.BaudRate = Integer.Parse(_baudRate)
            'BaudRate
            If _dataBits <> String.Empty Then comPort.DataBits = Integer.Parse(_dataBits)
            'DataBits
            If _stopBits <> String.Empty Then comPort.StopBits = DirectCast([Enum].Parse(GetType(StopBits), _stopBits), StopBits)
            'StopBits
            If _parity <> String.Empty Then comPort.Parity = DirectCast([Enum].Parse(GetType(Parity), _parity), Parity)
            'Parity
            If _portName <> String.Empty Then comPort.PortName = _portName
            'PortName
            'now open the port
            comPort.Open()
            'display message
